...
const AdminLogin = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await axios.post('http://192.168.1.42:19001/adminLogin', {
        email: email,
        password: password,
      });
      ...
      if (success) {
        ...
        navigation.navigate('DisplayDetails'); // Replace 'MainScreen' with the name of the screen you want to navigate to
      } else {
        setMessage(message);
      }
    } catch (error) {
